...
async function runSfePrep(): Promise<void> {
  const cwd = process.cwd();
  const configPath = path.join(cwd, "lumora.config.ts");
  let config;
  try {
  ...
  for (const f of fileNames) {
    payload[f] = await readFile(path.join(dir, f), "utf8");
  }

  const outPath = path.join(dir, "_migrations_embedded.ts");
  const code = `// GENERATED BY LUMORA SFE-PREP
export const embeddedFiles: Record<string, string> = ${JSON.stringify(payload, null, 2)};
`;
  await writeFile(outPath, code, "utf8");
  console.log(`\nGenerated ${outPath} with ${fileNames.length} embedded migration(s).`);
  console.log(`Pass 'embeddedFiles' from this module to 'migrations.embeddedFiles' in your config.`);
}
