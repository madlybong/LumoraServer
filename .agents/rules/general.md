...
| G5 | **Full Local Suite Before Branch Push** | Before pushing any branch, run `bun test` locally. If PostgreSQL is configured via `.env`, all 153 tests must pass. If not configured, 148 non-pg tests must pass and the remaining 5 are expected to fail with connection refused. Never push with unexpected test failures. |
